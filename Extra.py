...
def writeResult(model):
	...
	os.path.join('data/table_sante/', 'extratrees_summary_allergy.txt')
	file = open ('data/table_sante/extratrees_summary_allergy.txt', 'a')
	file.write('Summary: \n')
	file.write('-'*50)
	file.write('\n')
	...
	os.path.join('data/table_sante/', 'extratrees_best_pred_allergy.txt')
	np.savetxt('data/table_sante/extratrees_best_pred_allergy.txt', pred)
	file.write(str(pred))
	file.write('\n')
	file.write('\n')
	file.write('-'*50)
	...
	os.path.join('data/table_sante/', 'extratrees_best_proba_allergy.txt')
	np.savetxt('data/table_sante/extratrees_best_proba_allergy.txt', proba)
...
if __name__ == '__main__':	
	...
	os.path.join('data/table_sante/', 'pca_x_train_lda')
	np.savetxt('data/table_sante/pca_x_train_lda', df_X_train_pca)

	os.path.join('data/table_sante/', 'pca_x_test_lda')
	np.savetxt('data/table_sante/pca_x_test_lda', df_X_test_pca)
	"""

	print ("5. GrideSearchCV defining... \n")

	params = {'n_estimators':[5, 10, 30, 50, 100, 150], 'min_samples_split':[2, 5, 10], 'min_samples_leaf': [3, 5, 10, 15]}

	extraT = ExtraTreesClassifier(max_features=None, n_jobs=-1)
	model = grid_search.GridSearchCV(extraT, params, verbose=1, cv=30, n_jobs=-1)
