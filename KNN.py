...
def writeResult(model):
	...
	os.path.join('data/table_sante/', 'knn_summary_allergy.txt')
	file = open ('data/table_sante/knn_summary_allergy.txt', 'a')
	file.write('Summary: \n')
	file.write('-'*50)
	file.write('\n')
	...
	os.path.join('data/table_sante/', 'knn_best_pred_allergy.txt')
	np.savetxt('data/table_sante/knn_best_pred_allergy.txt', pred)
	file.write(str(pred))
	file.write('\n')
	file.write('\n')
	file.write('-'*50)
	...
	os.path.join('data/table_sante/', 'knn_best_proba_allergy.txt')
	np.savetxt('data/table_sante/knn_best_proba_allergy.txt', proba)
...
if __name__ == '__main__':	
	...
	params = {'n_neighbors':[1, 5, 10, 20, 30, 50, 100], 'weights':['uniform', 'distance'], 'metric': ['minkowski', 'euclidean', 'manhattan'], 'algorithm':['auto', 'kd_tree']}

	knn = KNC()
	model = grid_search.GridSearchCV(knn, params, verbose=1, cv=30, n_jobs=-1)
