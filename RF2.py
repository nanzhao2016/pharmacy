...
def writeResult(model):
	...
	os.path.join('data/table_sante/', 'rf_summary_allergy.txt')
	file = open ('data/table_sante/rf_summary_allergy.txt', 'a')
	file.write('Summary: \n')
	file.write('-'*50)
	file.write('\n')
	...
	os.path.join('data/table_sante/', 'rf_best_pred_allergy.txt')
	np.savetxt('data/table_sante/rf_best_pred_allergy.txt', pred)
	file.write(str(pred))
	file.write('\n')
	file.write('\n')
	file.write('-'*50)
	...
	os.path.join('data/table_sante/', 'rf_best_proba_allergy.txt')
	np.savetxt('data/table_sante/rf_best_proba_allergy.txt', proba)
...
if __name__ == '__main__':	
	...
	rf = RandomForestClassifier(max_features=None, n_jobs=-1)
	model = grid_search.GridSearchCV(rf, params, verbose=1, cv=30, n_jobs=-1)
