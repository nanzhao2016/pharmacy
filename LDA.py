...
def writeResult(model):
	...
	os.path.join('data/table_sante/', 'lda_summary_allergy2.txt')
	file = open ('data/table_sante/lda_summary_allergy2.txt', 'a')
	file.write('Summary: \n')
	file.write('-'*50)
	file.write('\n')
	...
	os.path.join('data/table_sante/', 'lda_best_pred_allergy2.txt')
	np.savetxt('data/table_sante/lda_best_pred_allergy2.txt', pred)
	file.write(str(pred))
	file.write('\n')
	file.write('\n')
	file.write('-'*50)
	...
	os.path.join('data/table_sante/', 'lda_best_proba_allergy2.txt')
	np.savetxt('data/table_sante/lda_best_proba_allergy2.txt', proba)
...
if __name__ == '__main__':	
	...
	params = [
		{'solver': ['svd']},
		{'solver': ['eigen'], 'shrinkage': [None, 'auto', 0, 0.3, 0.5, 0.8, 1], 'store_covariance': [True]}
	]

	mlda = LDA()
	model = grid_search.GridSearchCV(mlda, params, verbose=1, cv=30, n_jobs=-1)
